...
public class MainFrame extends JFrame {
    ...
    private final AcademyDataImportService academyDataImportService = new AcademyDataImportService();
    ...
    private void loadCsvFile() {
        ...
        try {
            ...
            showOperationError("파일 읽기 실패", "파일을 읽는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void updateDatabase() {
        ...
        try {
            ...
            showOperationError("DB 업데이트 실패", "데이터베이스 업데이트 중 오류가 발생했습니다.", ex);
            statusLabel.setText("DB 업데이트에 실패했습니다.");
        }
    }
    ...
    private void saveAndAppendLog(MessagePreview preview, String parentEmail, String status, String error) {
        try {
            ...
            preview.setStatus("FAILED");
            appendLog(preview, "발송 로그 저장 중 오류가 발생했습니다.");
            showOperationError("발송 로그 저장 실패", "발송 로그를 저장하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void refreshLogTableFromDatabase() {
        try {
            ...
            showOperationError("발송 로그 조회 실패", "발송 로그를 조회하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void searchLogTable() {
        try {
            ...
            showOperationError("발송 로그 검색 실패", "발송 로그를 검색하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void retrySelectedFailedLog(JTable logTable) {
        ...
        try {
            ...
            showOperationError("재시도 실패", "메일 재발송 처리 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void exportLogCsv() {
        try {
            ...
            showOperationError("CSV 저장 실패", "CSV 파일을 저장하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void loadTemplate() {
        try {
            ...
            showOperationError("템플릿 불러오기 실패", "템플릿을 불러오지 못해 기본 템플릿을 사용합니다.", ex);
        }
    }
    ...
    private void saveTemplate() {
        try {
            ...
            showOperationError("템플릿 저장 실패", "템플릿을 저장하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void refreshMakeupTables() {
        try {
            ...
            showOperationError("보강 데이터 조회 실패", "보강 데이터를 조회하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void loadSessionOptionsForSelectedAbsent(ListSelectionEvent event, JTable absentTable) {
        ...
        try {
            ...
            showOperationError("보강 가능 수업 조회 실패", "보강 가능 수업을 조회하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void requestSelectedMakeup(JTable absentTable, JTable sessionTable) {
        ...
        try {
            ...
            showOperationError("보강 신청 실패", "보강 신청을 처리하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void updateSelectedMakeupStatus(JTable requestTable, String status) {
        ...
        try {
            ...
            showOperationError("보강 상태 변경 실패", "보강 상태를 변경하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void refreshEnrollmentTable() {
        try {
            ...
            showOperationError("학생 관리 조회 실패", "학생 정보를 조회하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void saveSelectedEnrollment(JTable table) {
        ...
        try {
            ...
            showOperationError("학생 관리 저장 실패", "학생 정보를 저장하는 중 오류가 발생했습니다.", ex);
        }
    }
    ...
    private void saveEncryptedMailSettings() {
        ...
        try {
            ...
            showOperationError("메일 설정 저장 실패", "메일 설정을 저장하는 중 오류가 발생했습니다.", ex);
        } finally {
            Arrays.fill(masterPassword, '\0');
    ...
    private void showOperationError(String title, String userMessage, Exception ex) {
        LOGGER.log(Level.SEVERE, title, ex);
        JOptionPane.showMessageDialog(this, userMessage, title, JOptionPane.ERROR_MESSAGE);
    }
}
