...
이 저장소는 기존 Swing/Oracle 기능을 유지하면서 내부 품질을 개선한 **Refactoring 1 (버전 1.1.0)** 결과물입니다. PostgreSQL, Spring Boot, REST API, React 전환은 아직 구현하지 않았습니다.

백엔드는 비즈니스 규칙(`domain`), 유스케이스(`service`), 외부 경계(`port`), Oracle 구현(`dao`)으로 분리되어 있습니다. 리팩토링 근거는 [백엔드 요구사항](docs/BACKEND_REQUIREMENTS.md), [Business Rule Refactoring](docs/refactoring/01-business-readable-refactoring.md), [Backend Architecture Refactoring](docs/refactoring/02-backend-architecture-refactoring.md)을 참고하세요. 두 문서는 모두 Refactoring 1 안의 세부 작업입니다.
...
핵심 개선 사항은 문자열로 흩어진 상태와 업무 규칙의 도메인화, 화면과 유스케이스 분리, 포트를 통한 외부 의존성 경계 설정, DAO 연결 제공자 주입입니다. 기존 Java 17, Swing, Maven, Oracle JDBC, Jakarta Mail 구성과 `test 결과` 문자열 모델은 유지합니다.
...
5. 프로그램에서 Excel/CSV를 불러온 뒤 `DB 업데이트 실행`을 누릅니다.
...
1. **Database Redesign**: PostgreSQL 스키마 및 마이그레이션·롤백 설계
2. **Backend Modernization**: Spring Boot와 REST API 도입
3. **Web Frontend**: React 기반 사용자 화면 전환
4. **Production Readiness**: Docker, CI/CD, 보안 강화, 클라우드 배포
...
현재 단위 테스트는 도메인 상태 변환, 보강 자격·상태 전이, 메시지 구성, 입력 데이터 검증, CSV 수식 주입 방어를 확인합니다. Oracle DAO는 실제 Oracle 테스트 환경에서 별도 통합 검증이 필요합니다.
